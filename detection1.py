...
img = cv2.imread('run13.png')
gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
gray= cv2.bilateralFilter(gray,7,70,70)
...
(thresh, edges) = cv2.threshold(gray, 40, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
lines = cv2.HoughLines(gray1,1,np.pi/720, 50)
# 57 all filles and on 58 no lines ????
print('lines',lines)
print(lines.shape)

for i in range(len(lines)-1):
        if (np.abs(lines[i][0][1]-lines [i+1][0][1]) <=0.5 and np.abs(lines[i][0][0])>4):
          rho,theta=lines[i][0]
          a = np.cos(theta)        
          b = np.sin(theta)
          x0 = a*rho
